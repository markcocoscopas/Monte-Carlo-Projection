...
def mk_button(parent, text, command, font=FONT, bg=ACCENT, fg="white",
    ...
    """
    Cross-platform coloured button using a Label with bindings.
    Bypasses macOS tkinter button rendering restrictions entirely.
    """
    ...
    # Expose config so run_btn.config(state=...) still works
    def config(**kw):
        if "state" in kw:
            state = kw.pop("state")
            if state == "disabled":
                label.config(fg="#888888", **kw)
            else:
                label.config(fg=fg, **kw)
        if "text" in kw:
